...
def my_inverse(a, N):
    '''
        a, m : entiers
        retourne, s'il existe, un entier b tel que a*b = 1 mod n
    '''
    g, x, y = egcd(a, N)
    if g != 1:
        return str(a) + " n'est pas inversible modulo " + str(N)
    else:
        return x % N
...
def all_inverse(n):
    '''
        n : entier
        retourne la liste des entiers inversibles modulo n
    '''
    l = []
    for i in range(n):
        if my_inverse(i,n) != -1:
...
def my_expo_mod(base, exponent, modulus):
    '''
        base, exponent, modulus : entiers
        retourne g^n mod N avec une méthode d'exponentiation discrète binaire
    '''
    result=1
    while (exponent > 0):
        if ((exponent & 1) > 0):
...
def is_carmichael_naive(N):
    '''
        N : entier
        renvoie True si N est un entier de Carmichael, False sinon
    '''
    if N == 0 or N == 1:
        return False
    if first_test(N):
...
def liste_carmichael(n, b_time=False):
    '''
        liste les Carmichael inférieurs à n
    '''
    l = []
    if not b_time:
	for i in range(n):
...
def gen_charmichael(b_p, b_q, b_r):
    '''
        b_p, b_q, b_r : bornes supérieures respectives de p, q, r
        génère un nombre de Carmichael avec 3 facteurs premiers
    '''
    while True:
        p = random.randint(b_p)
        if first_test(b_p):
            break
    while True:
        q = random.randint(b_q)
        if first_test(b_q):
            break
    while True:
        r = random.randint(b_r)
        if first_test(b_r):
            break
...
#print("n = {}, premier = {}".format(17,miller_rabin2(17)))
#print(prim_base_a(90,83))
...
for j in range(1,h):
    ...
    print("b = {}".format(b))
print("b = {}".format(b))'''
print("i : {}, premier : {}".format(17, miller_rabin(17)))
'''
for i in range(4,100):
	print("i : {}, premier : {} ".format(i, miller_rabin(i)))
'''
'''
for i in range(4,100):
	print("i : {}, premier : {} ".format(i, miller_rabin2(i)))
